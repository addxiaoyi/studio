...
> 基于 Anthropic、LangChain、OpenAI 三大顶级 AI 公司的工程博客、官方文档和实战经验，对比 Helstera 现有 Agent 架构，提炼关键设计原则和改进方向。
>
> 日期：2026-03-27
...
- [第二部分：Helstera 现有架构深度分析](#第二部分helstera-现有架构深度分析)
...
## 第二部分：Helstera 现有架构深度分析

### 架构总览

Helstera 的 Agent 系统基于 **deepagents 框架**（而非原始 LangGraph），围绕 canvas 操作构建：
...
位于 `apps/server/src/agent/prompts/helstera-main.ts`（约 107 行，中文）：
...
**建议**：为 Helstera Agent 添加 think 工具，特别是在以下场景触发：
...
**建议**：当前 Helstera 工具数量较少（5-6 个），暂不紧迫。但随着功能扩展（如添加文本编辑、动画、导出等工具），应提前规划按需加载机制。
...
> **核心结论**：Helstera 的 Agent 架构在核心编排模式（Supervisor + Sub-agent）、工具设计（canvas 操作）和异步处理（PGMQ）方面已经建立了良好的基础。最大的提升空间在于**提示词工程的精细化**（Few-Shot Examples、Think Tool、错误处理指导）和**自动验证环节**（Reflection Loop）——这些都是低成本高回报的改进。中长期应关注 Guardrails 体系和系统化评估 pipeline 的建设。
