...
import type { SkillCategory, SkillDetail, SkillListItem } from "@helstera/shared";
...
export default function SkillsPage() {
  ...
  return (
    <div className="px-4 py-6 sm:px-6 md:p-8">
      {/* Header */}
      <h1 className="text-base font-medium sm:text-lg">Skills</h1>
      ...
      <div className="mb-4 flex items-center gap-1 overflow-x-auto border-b border-border sm:mb-6">
        {TABS.map((tab) => (
          <button
            ...
            className={cn(
              "min-h-[44px] whitespace-nowrap px-3 py-2 text-sm font-medium transition-all duration-300 border-b-2 -mb-px sm:min-h-0",
              activeTab === tab
      ...
      {activeTab === "installed" && (
        <>
          ...
          <div className="mb-4 flex flex-wrap items-center gap-2 sm:mb-6 sm:gap-3">
            ...
            <div className="relative order-last w-full sm:order-none sm:max-w-sm sm:flex-1">
              ...
              <input
                ...
                aria-label="搜索技能"
                className="h-10 w-full rounded-lg border border-input bg-transparent pl-8 pr-3 text-sm outline-none transition-all duration-300 placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 sm:h-7"
              />
            </div>
          ...
          <motion.div
            ...
            className="mb-4 flex items-center gap-3 rounded-2xl glass-soft border border-border/40 p-4 sm:mb-6 sm:gap-5 sm:p-5"
          >
            {/* Puzzle illustration */}
            <div className="hidden sm:flex shrink-0 items-center justify-center">
              <div className="relative h-16 w-20">
                {/* Card 1 */}
                <div className="absolute left-0 top-1 h-14 w-12 rounded-xl glass border border-border/40 shadow-soft" />
                {/* Card 2 (overlapping) */}
                <div className="absolute left-5 top-0 h-14 w-12 rounded-xl glass border border-border/60 shadow-card flex items-center justify-center">
                  <svg
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
          ...
          {filteredSkills.length === 0 ? (
            ...
            >
              <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-full glass-soft">
                <Search className="size-5 text-foreground/70" strokeWidth={1.5} />
              </div>
