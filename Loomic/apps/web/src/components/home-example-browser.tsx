...
function ExampleChip({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "inline-flex h-8 items-center gap-1.5 rounded-full border px-3.5 text-xs transition-all duration-300",
        accent === "special"
          ? "border-accent/30 glass-soft text-foreground hover:glass"
          : active
            ? "border-transparent bg-foreground text-background"
            : "border-border/40 glass-soft text-muted-foreground hover:glass hover:text-foreground",
        disabled && accent !== "special" && "cursor-not-allowed opacity-50",
      )}
    >
      {accent === "special" && (
...
function ExamplePreviewCard({
  ...
  return (
    <button
      ...
      className={cn(
        "group aspect-[217/130] overflow-hidden rounded-2xl p-4 text-left transition-all duration-300",
        selected
          ? "glass shadow-card ring-1 ring-foreground/15"
          : "glass-soft shadow-soft hover:-translate-y-0.5 hover:glass hover:shadow-card",
      )}
    >
