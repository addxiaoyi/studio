...
function Popover({ open, triggerRef, onClose, children, className: extraClass }: {
  ...
  return createPortal(
    <div ref={containerRef} style={pos}
      className={`rounded-2xl glass border border-border/60 shadow-card-hover animate-in fade-in slide-in-from-bottom-2 duration-150 ${extraClass ?? "p-2"}`}
...
/* ── Toolbar button ── */
const btnClass =
  "flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-muted-foreground hover:bg-foreground/[0.05] hover:text-foreground transition-all duration-200";
...
function ElementRow({ el, onSelect }: { el: ExcalidrawEl; onSelect: (id: string) => void }) {
  return (
    <button
      type="button"
      className="flex items-center gap-2 w-full px-2.5 py-1.5 text-xs rounded-xl hover:bg-foreground/[0.04] transition-all duration-200 text-foreground text-left"
      onClick={() => onSelect(el.id)}
    >
      <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded border border-border/50 glass-soft text-[10px] leading-none font-light">
        {elTypeIcon(el.type)}
      </span>
...
export function CanvasBottomBar({ excalidrawApi, layersOpen, onToggleLayers, filesOpen, onToggleFiles, leftPanelOpen }: CanvasBottomBarProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-0.5 rounded-full glass-soft border border-border/40 px-1 py-1 shadow-soft">
        ...
        <button ref={zoomBtnRef} type="button" className="min-w-[40px] text-center text-xs text-muted-foreground select-none cursor-pointer hover:text-foreground transition-all duration-300" onClick={toggleZoomMenu}>
          {Math.round(zoom * 100)}%
        </button>
      ...
      <Popover open={zoomMenuOpen} triggerRef={zoomBtnRef} onClose={() => setZoomMenuOpen(false)}>
        <div className="flex flex-col gap-0.5 min-w-[100px]">
          {ZOOM_PRESETS.map((v) => (
            <button key={v} type="button" className="px-3 py-1.5 text-xs text-left rounded-md hover:bg-foreground/[0.04] transition-all duration-300 text-foreground" onClick={() => handleZoomTo(v)}>{Math.round(v * 100)}%</button>
          ))}
          <div className="h-px bg-border my-0.5" />
          <button type="button" className="px-3 py-1.5 text-xs text-left rounded-md hover:bg-foreground/[0.04] transition-all duration-300 text-foreground" onClick={handleFitAll}>Fit All</button>
        </div>
      </Popover>
      ...
      <Popover open={bgPickerOpen} triggerRef={bgBtnRef} onClose={() => setBgPickerOpen(false)} className="w-[260px] rounded-2xl p-3">
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <button type="button" className="flex h-5 w-5 items-center justify-center rounded text-muted-foreground hover:text-foreground transition-all duration-300"
          ...
          <div className="flex flex-wrap items-center gap-2">
            {BG_PRESETS.map((hex) => (
              <button key={hex} type="button"
                className={`h-6 w-6 shrink-0 rounded-full border hover:scale-110 transition-all duration-300 ${bgColor === hex ? "border-foreground ring-1 ring-foreground ring-offset-1 ring-offset-card" : "border-border"}`}
