...
import type { SkillCategory, SkillListItem, SkillSource } from "@helstera/shared";
...
function ToggleSwitch({
  ...
  return (
    <button
      ...
      className={cn(
        "relative inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full transition-all duration-300 duration-200 sm:h-5 sm:w-9",
        checked ? "bg-primary" : "bg-muted",
      )}
    >
      <motion.span
        className="pointer-events-none block h-4 w-4 rounded-full bg-foreground sm:h-3.5 sm:w-3.5"
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              render={
                <button
                  type="button"
                  className="rounded-md p-0.5 text-muted-foreground opacity-0 transition-all duration-300 hover:bg-foreground/[0.04] group-hover:opacity-100"
