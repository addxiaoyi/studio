...
import type { SkillCategory } from "@helstera/shared";
              ...
              <div className="space-y-3">
                {files.map((file, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className="relative rounded-lg border border-border bg-secondary/40 p-3 space-y-2.5"
                    >
                      {/* Remove button */}
                      <button
                        type="button"
                        onClick={() => removeFile(index)}
                        className="absolute top-2 right-2 p-0.5 rounded-md text-muted-foreground hover:text-foreground hover:bg-foreground/[0.04] transition-all duration-300"
                        aria-label="删除文件"
                      >
