...
export function CanvasAIToolbar({
  ...
  return (
    <>
      ...
      <div
        className="absolute top-3 right-3 z-20 flex gap-0.5 rounded-xl p-1 glass-soft border border-border/40 shadow-card"
      >
        <button
          onClick={() =>
            setActivePanel(activePanel === "image" ? null : "image")
          }
          className={`flex items-center justify-center h-8 w-8 rounded-lg text-sm transition-all duration-300 cursor-pointer ${
            activePanel === "image"
        ...
        <button
          ...
          className="flex items-center justify-center h-8 w-8 rounded-lg text-sm transition-all duration-300 cursor-pointer text-foreground/70 hover:bg-foreground/[0.04] hover:text-foreground"
          title="AI Video"
        >
          <svg
            className="h-4 w-4"
