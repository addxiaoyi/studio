...
export function ImageGeneratorPanel({
  ...
  return createPortal(
    <div
      ...
      className="fixed z-[100] w-[450px] rounded-2xl border border-border/60 glass-soft p-2 shadow-card-hover backdrop-blur-lg"
        ...
        <div className="flex items-center">
          ...
          <div className="relative">
            <button
              ...
              className="flex h-8 items-center gap-1 rounded-lg px-2 text-xs text-muted-foreground transition-all duration-300 hover:bg-foreground/[0.04] focus-visible:ring-2 focus-visible:ring-foreground/20 ?"
            >
            ...
            {showModelDropdown && (
              <div className="absolute bottom-full left-0 z-50 mb-1 max-h-[280px] w-[260px] overflow-y-auto rounded-2xl border border-border/60 glass py-1 shadow-soft">
                {models.map((m) => (
                  <button
                    key={m.id}
                    type="button"
                    onClick={() => handleModelChange(m.id)}
                    className={`flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs transition-all duration-300 hover:bg-foreground/[0.04] focus-visible:ring-2 focus-visible:ring-foreground/20 ?${m.id === model ? "bg-muted" : ""} ${m.accessible === false ? "opacity-60" : ""}`}
                  >
                    {m.iconUrl && (
                      <img
                        src={m.iconUrl}
          ...
          <button
            ...
            className={`flex h-8 w-8 items-center justify-center rounded-lg transition-all duration-300 hover:bg-foreground/[0.04] focus-visible:ring-2 focus-visible:ring-foreground/20 ?${
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="relative">
            <button
              ...
              className="flex h-8 items-center gap-0.5 rounded-lg px-2 text-xs text-muted-foreground transition-all duration-300 hover:bg-foreground/[0.04] focus-visible:ring-2 focus-visible:ring-foreground/20 ?"
            >
              <span className="text-foreground">
            ...
            {showQualityDropdown && (
              <div className="absolute bottom-full right-0 z-50 mb-1 rounded-2xl border border-border/60 glass py-1 shadow-soft">
                {QUALITIES.map((q) => (
                  <button
                    key={q.value}
                    type="button"
                    onClick={() => handleQualityChange(q.value)}
                    className={`flex w-full items-center gap-2 px-3 py-1.5 text-xs transition-all duration-300 hover:bg-foreground/[0.04] focus-visible:ring-2 focus-visible:ring-foreground/20 ?${q.value === quality ? "bg-muted text-foreground" : "text-muted-foreground"}`}
                  >
                    {q.label}
                  </button>
                ))}
              ...
              <div className="absolute bottom-full right-0 z-50 mb-1 rounded-2xl border border-border/60 glass py-1 shadow-soft">
                {ASPECT_RATIOS.map((r) => (
                  <button
                    ...
                    className={`flex w-full items-center px-3 py-1.5 text-xs transition-all duration-300 hover:bg-foreground/[0.04] focus-visible:ring-2 focus-visible:ring-foreground/20 ?${r === aspectRatio ? "bg-muted text-foreground" : "text-muted-foreground"}`}
                  >
                    {r}
                  </button>
                ))}
          ...
          <button
            ...
            className="flex h-8 min-w-12 items-center justify-center gap-1 rounded-full bg-primary p-2 text-primary-foreground transition-all duration-300 hover:bg-primary/80 hover:accent-glow disabled:cursor-not-allowed disabled:bg-muted disabled:text-muted-foreground"
          >
            {loading ? (
