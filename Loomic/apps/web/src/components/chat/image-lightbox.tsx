...
function LightboxBtn({
  ...
  return (
    <button
      ...
      title={title}
      onClick={onClick}
      className="flex h-8 w-8 items-center justify-center rounded-full text-white/80 transition-all duration-300 hover:bg-white/15 hover:text-white"
    >
      <svg
  ...
  return createPortal(
    ...
    >
      ...
      <button
        ...
        title="\u5173\u95ed (Esc)"
        onClick={onClose}
        className="absolute top-4 right-4 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-black/40 text-white/80 backdrop-blur-md transition-all duration-300 hover:bg-black/60 hover:text-white"
      >
        <svg
          className="h-5 w-5"
          viewBox="0 0 24 24"
  ...
  return (
    <>
      <span
        ...
        className="inline-flex h-[22px] items-center gap-1 rounded-md px-1 mx-0.5 border-[0.5px] border-muted-foreground text-foreground hover:bg-foreground/[0.04] cursor-pointer align-middle"
        role="button"
        tabIndex={0}
        onKeyDown={(e) => {
        ...
        createPortal(
          ...
          >
            <img
              src={src}
              alt={name}
              // bg-card provides a solid backdrop for transparent PNGs/animated images
