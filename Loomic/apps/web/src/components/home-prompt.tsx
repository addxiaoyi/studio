...
import type { ImageGenerationPreference, VideoGenerationPreference } from "@helstera/shared";
...
export const HomePrompt = forwardRef<HomePromptHandle, HomePromptProps>(
  function HomePrompt(
    ...
    return (
      <div className="overflow-hidden rounded-2xl border border-border/60 glass-soft shadow-soft transition-[border,box-shadow] duration-300 focus-within:border-foreground/20 focus-within:shadow-card">
          ...
          <div className="flex flex-col gap-3 border-b border-border/60 px-4 py-3">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="inline-flex items-center rounded-full glass-soft border border-border/40 px-2 py-1 text-[11px] font-medium text-muted-foreground">
              ...
              {onClearSelectedSeed ? (
                <button
                  ...
                  className="shrink-0 rounded-full glass-soft border border-border/40 px-2.5 py-1 text-[11px] text-muted-foreground transition-all duration-300 hover:text-foreground focus-visible:ring-2 focus-visible:ring-foreground/20"
                >
                  清除
                </button>
              ) : null}
        ...
        <textarea
          ...
          onPaste={handlePaste}
          onInput={handleInput}
          placeholder="让 Helstera 帮你设计..."
          disabled={disabled}
          rows={2}
          className="w-full resize-none bg-transparent px-4 pt-4 pb-3 text-sm leading-relaxed text-foreground placeholder:text-muted-foreground focus:outline-none disabled:opacity-50"
        />
        ...
        <div className="flex items-center justify-between px-2 pb-2 sm:px-3 sm:pb-3">
            ...
            {onAddFiles ? (
              <>
                ...
                <button
                  ...
                  title="Attach"
                  className="flex h-10 w-10 items-center justify-center rounded-full border border-border/60 text-foreground transition-all duration-200 hover:bg-foreground/[0.04] focus-visible:ring-2 focus-visible:ring-foreground/20 sm:h-8 sm:w-8"
                >
                  <svg
              ...
              <button
                ...
                disabled
                title="Attach"
                className="flex h-10 w-10 cursor-not-allowed items-center justify-center rounded-full border border-border/60 text-foreground opacity-30 sm:h-8 sm:w-8"
              >
                <svg
                  className="h-[14px] w-[14px]"
                  viewBox={toolbarButtons[0].viewBox}
          ...
          <div className="flex items-center gap-2">
                  ...
                  return (
                    <div key={btn.name} className="relative">
                      <button
                        ...
                        title={btn.name}
                        className={`flex h-10 w-10 items-center justify-center rounded-full border transition-all duration-200 focus-visible:ring-2 focus-visible:ring-foreground/20 sm:h-8 sm:w-8 ${
            ...
            <button
              ...
              aria-label="发送"
              className={`flex h-10 w-10 items-center justify-center rounded-full transition-all focus-visible:ring-2 focus-visible:ring-foreground/20 sm:h-8 sm:w-8 ${
                hasContent && !disabled && !isUploading
                  ? "bg-foreground text-background hover:scale-[1.05] active:scale-95"
                  : "cursor-not-allowed bg-foreground text-background opacity-30"
              }`}
            >
              <svg
                className="h-[14px] w-[14px]"
