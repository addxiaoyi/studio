...
import type { BackgroundJobStatus, BackgroundJobType, ImageQualityLevel } from "@helstera/shared";
import {
  ...
  app.post("/api/jobs/image-generation", async (request, reply) => {
    try {
      ...
      if (options.creditService && options.tierGuard) {
        // Top-up credit system: no plan-based restrictions.
        const quality: ImageQualityLevel = "hd";
        options.tierGuard.checkModelAccess(model);
