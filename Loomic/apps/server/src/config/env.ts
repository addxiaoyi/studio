...
export function loadServerEnv(
  ...
  // Explicit HELSTERA_AGENT_MODEL always takes precedence; otherwise fall back
  // to Gemini 2.5 Flash when only Google/Vertex is configured.
  const explicitModel =
    overrides.agentModel ?? parseAgentModel(source.HELSTERA_AGENT_MODEL);
  ...
  return {
    ...
    port: overrides.port ?? parsePort(source.HELSTERA_SERVER_PORT ?? source.PORT),
    version: overrides.version ?? readServerVersion(),
    webOrigin:
      overrides.webOrigin ?? source.HELSTERA_WEB_ORIGIN ?? DEFAULT_WEB_ORIGIN,
...
function parseAgentBackendMode(rawMode: string | undefined): AgentBackendMode {
  ...
  throw new Error(`Invalid HELSTERA_AGENT_BACKEND_MODE value: ${rawMode}`);
}
...
function parsePort(rawPort: string | undefined) {
  ...
  if (!Number.isInteger(port) || port <= 0) {
    throw new Error(`Invalid HELSTERA_SERVER_PORT value: ${rawPort}`);
  }
