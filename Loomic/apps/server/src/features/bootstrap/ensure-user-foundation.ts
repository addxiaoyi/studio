...
export function createViewerService(options: {
  ...
  return {
    async ensureViewer(user) {
      ...
      if (!workspace) {
        console.error('[bootstrap] workspace not found for user:', user.id);
        throw new BootstrapError();
      }
      ...
      if (!profile || !membership) {
        console.error('[bootstrap] profile/membership missing:', { profile, membership, userId: user.id, workspaceId: workspace.id });
        throw new BootstrapError();
      }
