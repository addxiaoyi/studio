// Payment lifecycle: legacy Lemon Squeezy subscription flow (kept for backward compat; new purchases use topup)
...
export type PaymentService = {
  ...
  /** Create a Lemon Squeezy checkout for a credit top-up package (USD). */
  createTopupCheckout(
    workspaceId: string,
    variantId: string,
    redirectUrl: string,
...
export function createPaymentService(options: {
  ...
  return {
    ...
    async createTopupCheckout(workspaceId, variantId, redirectUrl) {
      if (!variantId) {
        throw new Error(
          "No Lemon Squeezy variant configured for this topup package. Set LEMON_SQUEEZY_TOPUP_VARIANT_<id> in env.",
        );
      }
      const result = await lemonSqueezy.createCheckout(
        variantId,
    ...
    async handleWebhookEvent(eventName, payload) {
      ...
      switch (eventName) {
        case "subscription_created": {
          ...
          const plan: SubscriptionPlan = resolved?.plan ?? "free";
...
async function grantMonthlyCredits(
  ...
  // Top-up system: no monthly grants anymore.
  // (Kept as a stub for back-compat with Lemon Squeezy webhook.)
  if (plan === "free") return;
  // No-op: customers now buy credits via top-ups, not subscriptions.
  return;
}
