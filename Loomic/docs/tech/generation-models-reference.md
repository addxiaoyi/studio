# Helstera 图片/视频生成模型参考手册

> 覆盖 Helstera 支持的全部图片和视频生成模型，包括每个模型的原始参数、工具层归一化方案、以及 Provider 层的参数映射逻辑。
...
项目统一使用共享的 `getVideoCreditCost()` 计算预估和实际扣分。Metaso H3 按“每秒费率 × 生成秒数”计费，并向上取整为整数 Helstera 积分；人民币区间只用于价格透明展示，不参与扣款。
