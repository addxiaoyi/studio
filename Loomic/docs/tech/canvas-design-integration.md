...
```bash
# .env.local additions
HELSTERA_SANDBOX_ROOT=/tmp/helstera-sandbox-dev
HELSTERA_SKILLS_ROOT=./skills
```
...
- Skills + fonts copied to `/opt/helstera/skills/`
...
| Issue | Cause | Fix |
|-------|-------|-----|
| `execute` tool not available | Backend not sandbox | Check `HELSTERA_AGENT_BACKEND_MODE=state` and backend factory returns LocalShellBackend |
| Fonts not found | Wrong FONT_DIR | Check `HELSTERA_SKILLS_ROOT` env var |
