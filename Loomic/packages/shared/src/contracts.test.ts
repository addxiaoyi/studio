...
describe("@helstera/shared contracts", () => {
  it("shares the health response schema for server and web", () => {
    ...
    expect(parsed.service).toBe("helstera-server");
  });
  ...
  it("shares the viewer bootstrap contract for GET /api/viewer", () => {
    ...
    const parsed = viewerResponseSchema.parse({
      profile: {
        id: "user_123",
        email: "maker@helstera.test",
        displayName: "Helstera Maker",
  ...
  it("exposes supabase generated tables in the Database type", () => {
    // The generated database.ts should declare a Database type with
    // nested Tables under public. We just sanity-check the top-level shape.
    expect(databaseTypeSource).toMatch(/type Database\s*=/);
    expect(databaseTypeSource).toMatch(/Tables:/);
    expect(databaseTypeSource).toMatch(/public:/);
  });
});
