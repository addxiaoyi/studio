...
test("workspace includes apps and packages entries", async () => {
  const workspace = await readText("pnpm-workspace.yaml");

  // Updated to match explicit app paths instead of glob patterns
  assert.match(workspace, /apps\/web/);
  assert.match(workspace, /apps\/server/);
...
// Web and Server apps have full development workflow
...
// Desktop app uses placeholder scripts (tsc + vite only)
test("desktop app has valid build/typecheck scripts", async () => {
  const manifest = await readJson("apps/desktop/package.json");
...
test("@helstera/config exports a single low-drift package contract", async () => {
...
test("shared package placeholder exists for the upcoming contract task", async () => {
  ...
  assert.equal(manifest.name, "@helstera/shared");
